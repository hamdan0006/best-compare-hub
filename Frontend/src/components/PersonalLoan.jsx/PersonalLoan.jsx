...
function PersonalLoan() {
  ...
  const calculateMortgage = () => {
    ...
    if (downPaymentPercent > 100) {
      toast.error("Down payment percentage cannot exceed 100%.");
      return;
    }
if(propertyValue > 3000000){
  toast.error("Loan Amount Cannot exceed 3000000");
  return;

}
    // Validate loan period
    if (loanPeriod > 4) {
      toast.error("Loan period cannot exceed 4 years.");
      return;
    }

    // Ensure down payment is less than property value
    if (downPayment >= propertyValue) {
      toast.error("Down payment equals or exceeds the property value. Loan amount must be greater than 0.");
      setMonthlyPayment(0);
      return;
    }

    const totalMonths = loanPeriod * 12;
    const monthlyRate = 3 / 100 / 12; // Fixed annual interest rate of 3% divided by 12
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/personal-loan/apply-now`, {
        method: "POST",
        headers: {
      ...
      <div className="row justify-content-center w-100">
        ...
        <div className="col-lg-5 col-md-8 col-sm-10 custom-apply-column">
          <div style={{ width: "100%", margin: "0 auto", padding: "20px" }}>
            <h1 className="main-calc-heading">Personal Loan Calculator</h1>
